...
<p style="background-color:pink"><b>NOTE</b>: Spaces are significant in HYPER commands. The above example shows spaces are properly handed within embeded quotes (<code>STACK PUSH {"title":"just another one","tags":"with-test","completeFlag":"false"}</code>). However, there must be no spaces <i>between</i> elements in the JSON segments.</p>
